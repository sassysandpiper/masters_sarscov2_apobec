...
dir <- "C:/Users/1044832/Desktop/Analysis/3. Sequence change (against outlier)/clade_other coronaiviruses/aligned coronavirus"
...
for (df in dflist) {
  ...
  consensus_threshold <- (ncol(currentdf)-7) / 4 * 3
...
for (df in dflist) {
  currentdf <- get(df)
  write.csv(currentdf, paste(df, ".wrangled.csv"), row.names = FALSE)
}
