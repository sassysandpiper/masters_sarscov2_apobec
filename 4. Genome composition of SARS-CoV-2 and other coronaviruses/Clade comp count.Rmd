...
plot_data[1,1] <- "WIV04"
plot_data[, 3:6] <- plot_data[, 3:6] * 100




palette <- c("WIV04" = "#000000",
             "S" = "#669900",
             "O" = "#99CC33",
             "L" = "#CCEE66",
             "V" = "#006699",
...
ggplot(plot_data, aes(x = data_id , y = c_ratio_mean, fill = data_id)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  labs(title = "%C in Genome Composition", y = "Value", x = "Variant")+
  scale_x_discrete(limits = c("WIV04", "S", "L", "V", "O", "G", "GV", "GH", "GR", "GRY", "GK", "GRA"))+
  scale_fill_manual(values = palette, breaks = c("S", "L", "V", "O", "G", "GV", "GH", "GR", "GRY", "GK", "GRA"))+
  coord_cartesian(ylim = c(18.25, 18.40))

ggsave(plot = last_plot(), filename = "C by clade.png")  

  ```

```{r}
#plotting data in box plot
  
#Read data back in
G_nucleotide_composition<- read.csv("data_used_for_graphs/G_nucleotide_composition.csv")
GH_nucleotide_composition <- read.csv("data_used_for_graphs/GH_nucleotide_composition.csv")
GK_nucleotide_composition <- read.csv("data_used_for_graphs/GK_nucleotide_composition.csv")
GR_nucleotide_composition <- read.csv("data_used_for_graphs/GR_nucleotide_composition.csv")
GRA_nucleotide_composition <- read.csv("data_used_for_graphs/GRA_nucleotide_composition.csv")
GRY_nucleotide_composition <- read.csv("data_used_for_graphs/GRY_nucleotide_composition.csv")
GV_nucleotide_composition <- read.csv("data_used_for_graphs/GV_nucleotide_composition.csv")
L_nucleotide_composition <- read.csv("data_used_for_graphs/L_nucleotide_composition.csv")
O_nucleotide_composition <- read.csv("data_used_for_graphs/O_nucleotide_composition.csv")
S_nucleotide_composition <- read.csv("data_used_for_graphs/S_nucleotide_composition.csv")
V_nucleotide_composition <- read.csv("data_used_for_graphs/V_nucleotide_composition.csv")



G_nucleotide_composition$variant <- rep("G", nrow(G_nucleotide_composition))
GH_nucleotide_composition$variant <- rep("GH", nrow(GH_nucleotide_composition))
GK_nucleotide_composition$variant <- rep("GK", nrow(GK_nucleotide_composition))
GR_nucleotide_composition$variant <- rep("GR", nrow(GR_nucleotide_composition))
GRA_nucleotide_composition$variant <- rep("GRA", nrow(GRA_nucleotide_composition))
GRY_nucleotide_composition$variant <- rep("GRY", nrow(GRY_nucleotide_composition))
GV_nucleotide_composition$variant <- rep("GV", nrow(GV_nucleotide_composition))
L_nucleotide_composition$variant <- rep("L", nrow(L_nucleotide_composition))
O_nucleotide_composition$variant <- rep("O", nrow(O_nucleotide_composition))
S_nucleotide_composition$variant <- rep("S", nrow(S_nucleotide_composition))
V_nucleotide_composition$variant <- rep("V", nrow(V_nucleotide_composition))


G_nucleotide_composition$X <- NULL
GH_nucleotide_composition$X <- NULL
GK_nucleotide_composition$X <- NULL
GR_nucleotide_composition$X <- NULL
GRA_nucleotide_composition$X <- NULL
GRY_nucleotide_composition$X <- NULL
GV_nucleotide_composition$X <- NULL
L_nucleotide_composition$X <- NULL
O_nucleotide_composition$X <- NULL
S_nucleotide_composition$X <- NULL
V_nucleotide_composition$X <- NULL
...
plot.data2 <- rbind(G_nucleotide_composition, GH_nucleotide_composition, GK_nucleotide_composition, GR_nucleotide_composition, GRA_nucleotide_composition, GRY_nucleotide_composition, GV_nucleotide_composition, L_nucleotide_composition, O_nucleotide_composition, S_nucleotide_composition, V_nucleotide_composition)

plot.data2[, 2:5] <- plot.data2[, 2:5] * 100


ggplot(plot.data2, aes(x = variant, y = c_ratio, fill = variant))+
  geom_boxplot(outlier.shape = NA)+
  theme_bw()+
  labs(y = "%C", x = NULL)+
  scale_x_discrete(limits = c("L", "S", "V", "O", "G", "GV", "GH", "GR", "GRY", "GK", "GRA"))+
  scale_fill_manual(values = palettec, breaks = c("L", "S", "V", "O", "G", "GV", "GH", "GR", "GRY", "GK", "GRA"))+
  geom_hline(yintercept = 18.37342, linetype = "dashed", color = "red")+
  coord_cartesian(ylim = c(18.20, 18.50))+
  theme(axis.text.y = element_text(size = 13))+
  theme(axis.text.x = element_text(size = 13, face = "bold"))+
  theme(axis.title.y = element_text(size = 14, face = "bold"))


ggsave(plot = last_plot(), filename = "C by clade.png", width = 8, height = 4, units = "in", dpi = 600)
...
# Get the list of data frames in the environment
dataframe_list <- ls(pattern="_nucleotide_composition")
...
for (df_name in dataframe_list) {
  ...
  # Perform t-tests for the specified columns
  a_ratio_result <- t.test(df$a_ratio, mu = 0.2992)
  t_ratio_result <- t.test(df$t_ratio, mu = 0.3209)
  g_ratio_result <- t.test(df$g_ratio, mu = 0.1961)
  c_ratio_result <- t.test(df$c_ratio, mu = 0.1837)
  
  # Create a new row with the test results
  result_row <- data.frame(
    DataFrame = df_name,
    a_ratio_p_value = a_ratio_result$p.value,
    a_ratio_df = a_ratio_result$parameter,
    t_ratio_p_value = t_ratio_result$p.value,
    t_ratio_df = t_ratio_result$parameter,
    g_ratio_p_value = g_ratio_result$p.value,
    g_ratio_df = g_ratio_result$parameter,
    c_ratio_p_value = c_ratio_result$p.value,
    c_ratio_df = c_ratio_result$parameter
  )
